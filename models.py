...
class FeedbackBlock(nn.Module):
    def __init__(self,
        ...
        for idx in range(self.n_groups):
            up_blocks.append(DeconvBlock(n_features, n_features, kernel_size, stride, padding, act_type=act_type,
                                         norm_type=norm_type))
            down_blocks.append(
                ConvBlock(n_features, n_features, kernel_size, stride, padding, act_type=act_type, norm_type=norm_type))
            if idx > 0:
                uptran_blocks.append(
                    ConvBlock(n_features * (idx + 1), n_features, 1, 1, act_type=act_type, norm_type=norm_type))
                downtran_blocks.append(
                    ConvBlock(n_features * (idx + 1), n_features, 1, 1, act_type=act_type, norm_type=norm_type))
...
class SRFBN(nn.Module):
    def __init__(self,
        ...
        self.out = DeconvBlock(n_features, n_features, kernel_size, stride, padding, act_type='prelu',
                               norm_type=norm_type)
